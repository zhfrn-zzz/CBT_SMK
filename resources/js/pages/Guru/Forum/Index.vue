...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div v-else-if="threads && threads.data.length > 0" class="divide-y overflow-hidden rounded-xl border bg-card">
                <Link
                    v-for="t in threads.data" :key="t.id"
                    :href="`/guru/forum/${t.id}`"
                    class="flex items-start gap-3 p-4 hover:bg-slate-50/50 transition-colors"
                >
