...
import type { BreadcrumbItem, QuestionBank, Subject } from '@/types';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit', href: `/guru/bank-soal/${props.questionBank.id}/edit` },
];

const form = useForm({
    name: props.questionBank.name,
    subject_id: String(props.questionBank.subject_id),
    description: props.questionBank.description ?? '',
});

function submit() {
    form.put(`/guru/bank-soal/${props.questionBank.id}`);
}
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 rounded-lg p-4">
            <PageHeader title="Edit Bank Soal" :icon="Database" />
